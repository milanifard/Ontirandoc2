 ...
 function CheckEndDateBiggerThanStartDate(startyear, startmonth, startday, endyear, endmonth, endday)
 {
 	if (startyear<endyear) return true;
	if (startyear==endyear && startmonth<endmonth) return true;
	if (startyear==endyear && startmonth==endmonth && startday<endday) return true;
	return false;
 }
...
// isHour (hour)                       True if string s is a valid Hour number.
// isMinute (minute)                     True if string s is a valid Minute number.
// isTime (hour, minute)               True if string arguments form a valid time.
...
var ShamsdaysInMonth = makeArray(12);
ShamsdaysInMonth[1]  = 31;
ShamsdaysInMonth[2]  = 31;   // must programmatically check this
ShamsdaysInMonth[3]  = 31;
ShamsdaysInMonth[4]  = 31;
ShamsdaysInMonth[5]  = 31;
ShamsdaysInMonth[6]  = 31;
ShamsdaysInMonth[7]  = 30;
ShamsdaysInMonth[8]  = 30;
ShamsdaysInMonth[9]  = 30;
ShamsdaysInMonth[10] = 30;
ShamsdaysInMonth[11] = 30;
ShamsdaysInMonth[12] = 30;
...
function isShamsDate (year, month, day)
{   // catch invalid years (not 2- or 4-digit) and invalid months and days.
    if (! (isYear(year, false) && isMonth(month, false) && isDay(day, false))) return false;
...
{
	if (isWhitespace(hour)) return false;
	if (!isNonnegativeInteger(hour)) return false;
...
{
	if (isWhitespace(minute)) return false;
	if (!isNonnegativeInteger(minute)) return false;
